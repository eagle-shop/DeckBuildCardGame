...
void View::drawABCcount(ABCCOUNT *abcCount){
    ...
    }
    {
        ...
        action.position = CGPointMake(m_scene.frame.size.width - 135, (m_scene.frame.size.height * 2) / 8);
    ...
    }
    {
        ...
        buy.position = CGPointMake(m_scene.frame.size.width - 135, ((m_scene.frame.size.height  * 2) / 8) - 20);
    ...
    }
    {
        ...
        coin.position = CGPointMake(m_scene.frame.size.width - 135, ((m_scene.frame.size.height * 2) / 8) - 40);
...
void View::drawPhaseSW(Phase phase){
    ...
    if(phase == PHASE_ACTION){
        ...
        rect.position = CGPointMake(10, (m_scene.frame.size.height * 2) / 7.5);
    ...
    if(phase == PHASE_BUY){
        ...
        SKSpriteNode *rect = [SKSpriteNode spriteNodeWithColor:SKColor.greenColor size:CGSizeMake(100, 20)];
        rect.anchorPoint = CGPointMake(0.0, 0.0);
        rect.position = CGPointMake(10, ((m_scene.frame.size.height * 2) / 7.5) - 30);
        [phase_ addChild:rect];
    ...
    if(phase != PHASE_OTHERPLAYER){
        ...
        SKSpriteNode *rect = [SKSpriteNode spriteNodeWithColor:SKColor.greenColor size:CGSizeMake(100, 20)];
        rect.anchorPoint = CGPointMake(0.0, 0.0);
        rect.position = CGPointMake(10, ((m_scene.frame.size.height * 2) / 7.5) - 60);
        [phase_ addChild:rect];
...
void View::drawOtherSW(Phase phase){
    ...
    
    {
        ...
        rect.anchorPoint = CGPointMake(0.0, 0.0);
        rect.position = CGPointMake(120, ((m_scene.frame.size.height * 2) / 7));
        [phase_ addChild:rect];
    ...
    }
    {
        ...
        SKSpriteNode *rect = [SKSpriteNode spriteNodeWithColor:SKColor.greenColor size:CGSizeMake(80, 20)];
        rect.anchorPoint = CGPointMake(0.0, 0.0);
        rect.position = CGPointMake(120, ((m_scene.frame.size.height * 2) / 7) - 30);
        [phase_ addChild:rect];
...
void View::drawSupplyCards(vector<SUPPLYCARDS> *supplyCards){
    ...
    [m_scene addChild:supply];
    
    CGFloat w = m_scene.frame.size.width / CARDW;
    CGFloat h = 0;
    ...
    for(auto i = 0; i < countY; i++){
        ...
        for(auto j = 0; j < countX; j++){
            ...
            CGPoint position = CGPointMake(j * w, (m_scene.frame.size.height - 20) - (h * i));
...
void View::drawTrashCards(CARDNUM cardNum){
    ...
    CGPoint position = CGPointMake(m_scene.frame.size.width * ((CARDW - 2.0) / CARDW), (m_scene.frame.size.height * 2) / 12);
    CGPoint anchor = CGPointMake(0.0, 0.0);
    SKSpriteNode *trashCard = drawCard(&cardName, position, anchor, m_scene, &cardName, DRAW_NOMAL);
...
void View::drawPlayerCards(PLAYERCARDS *playerCards){
    ...
    const CGFloat w = m_scene.frame.size.width / CARDW;
    {
        // 山札の親ノード作っておく
        ...
        [m_scene addChild:deck];
        
        // 山札の描画
        const string cardName = BACKCARD;
        CGPoint position = CGPointMake(w * (CARDW - 1.0), (m_scene.frame.size.height * 2) / 12);
        CGPoint anchor = CGPointMake(0.0, 0.0);
    ...
    }
    {
        ...
        [m_scene addChild:hand];
        
        // CARDW枚より多い場合は、間隔を詰める
        CGFloat n = w;
        if(playerCards->handCards.handCardsName.size() > CARDW){
            n = (m_scene.frame.size.width - w) / (playerCards->handCards.handCardsName.size() - 1);
        }
    ...
    {
        // プレイエリアを取得
        SKNode *playArea = [m_scene childNodeWithName:[NSString stringWithFormat:@"//%s", PLAYAREA.c_str()]];
        ...
        if(playerCards->playCardsName.size() > CARDW){
            n = (m_scene.frame.size.width - w) / (playerCards->playCardsName.size() - 1);
        }
...
void View::delNode(vector<const string*> *name){
    for(auto x : *name){
        // nilでも大丈夫
        SKNode *del = [m_scene childNodeWithName:[NSString stringWithFormat:@"//%s", x->c_str()]];
...
SKSpriteNode* View::drawCard(const string *cardName, CGPoint position, CGPoint anchor, SKNode *drawNode, const string *nodeName, DrawState drawState){
    ...
    sprite.xScale = (m_scene.frame.size.width / sprite.size.width) / CARDW;
